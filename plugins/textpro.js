...
let handler = async (m, { conn, args: [effect], text: txt, usedPrefix, command }) => {
  if (!effect) throw '✳️ Escribe .textpro <nombre del efecto>\n\n📌 Ejemplo:\n.textpro space elyaas\n\n╭─❑ 「 LIST EFFECT 」 ❑──\n' + effects.map(v => v.title).join('\n│ • ')
  ...
  if (typeof res == 'number') throw res == -1 ? `✳️ Efecto *${effect}* no encontrado` : `📌 Usar formato ${usedPrefix}${command} ${effect} ${new Array(res).fill('text').map((v, i) => v + (i ? i + 1 : '')).join('|')}`
  let result = await axios.get(res, {
    responseType: 'arraybuffer'
  })
  await m.reply('*Haciendo...*')
